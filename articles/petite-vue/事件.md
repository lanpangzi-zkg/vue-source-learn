...
```
这里需要注意的是exp的值，就是我们希望事件发生之后能执行的回调逻辑，由于支持行内表达式和方法名两种写法，那么最终生成的事件回调函数就会有不同，怎么各不同呢，下面分别分析一下；
1.行内表达式
...
function processDirective(el, raw, exp, ctx) {
    ...
    raw = raw.slice(0, raw.indexOf('.')); // 【.】后面的字符都被截取掉了，此时raw就变成了@click/v-on:click
    if(raw[0] === ':') {
        // bind指令，暂时未实现
    }
    if (raw[0] === '@') {
