...
......
<div style="font-weight: bold" :style="style"></div> // 渲染结果：<div style="font-weight: bold; color: blue;">
<div :style="arrayStyle"> // 渲染结果：<div style="color: red; font-size: 16px;">
......
```
源代码的处理可以分为三步：
* normalizeStyle处理返回样式对象styleObj
...
理清了思路和注意事项之后，再去看源码就事半功倍了，这里就不贴代码了，具体可[查看](https://github.com/lanpangzi-zkg/vue-source-learn/blob/main/articles/petite-vue/其他指令.md)。到这里就说完v-model和v-bind两个指令了，当然还有其他诸如`v-html`、`v-text`等指令，通过前面一系列文章的讲解，再看这些指令就比较简单了，关于petite-vue指令部分的介绍就到此为止，后面将从全局的角度分析petite-vue的设计与流程。
